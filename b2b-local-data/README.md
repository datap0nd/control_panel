...
2. The installer reads the GitHub token from the `DG_GITHUB_TOKEN` environment variable that the data-governance installer already uses on work PCs. Nothing else is needed. If that variable is absent, create a `.env` file in the same folder with one line, `DG_GITHUB_TOKEN=<the token>`.
